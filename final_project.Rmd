...
Justification: Especially for invasive species, need to predict where they might go in order to take preventative measures.
...
- Using binary data ("is toad here?" instead of "how many toads are here") for RFs apparently worse
- In the lingo of the article, what I'm doing is using background data?? (not "pseudoabsence" data), I'm confused by second paragraph of intro. Possible explanation here https://rspatial.org/raster/sdm/3_sdm_absence-background.html
...
Take all species in ALA:
  ...
  3. Run random forest
  4. Assign affinity of species for cell using results (range of 0-1)
  5. Create raster brick for these affinities, each layer is a year (increasing temperature using rnorm() for each year in accordance with global warming,
  or using BOM data on it if can find)
  6. Create and run agent based model to see how cane toad distributions might look x years into the future
  7. Validate method by using data up to the year x (where $x \approx 2000$? and trying to predict distribution in 2024.
