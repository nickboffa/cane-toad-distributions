...
These attributes behaviours (apart from reproduction) are implemented in the following 'Toad' object.
...
1. Each tick, each cell can only have at most $6$ toads on it. Extras are killed using the following function:
...
2. If a cell is surrounded by cells (king's move) with $\geq6$ toads, then toads on that cell won't update (move, change energy, etc.) the next time-step. These cells and toads are called "fixed". To check if a cell is fixed:
...
for (tick in tick:100) {
  ...
  fixed_cells <- focal(tracker, w=matrix(1, 3, 3), fun=function(cell) is_fixed(cell, max_toads_per_cell)) #pad=TRUE, padValue=NA)
  ...
  cat(paste("Tick: ", tick, "\n",
            "Alive toads: ", length(alive_toads), "\n",
            "Moving toads: ", length(moving_toads), " (", round(100*length(moving_toads)/length(alive_toads)), "%)\n",
            "Cells covered: ", cellStats(flatten(tracker), 'sum'), "\n",
            "Percentage of mainland covered: ", round(100*cellStats(flatten(tracker), 'sum')/n_cells_in_map), "% \n\n", sep = ""))
}
...
processing file: final_project.Rmd
                                                                                                               

Quitting from lines 389-423 [unnamed-chunk-21] (final_project.Rmd)
Error in `setValues()`:
! length(values) is not equal to ncell(x), or to 1
Backtrace:
 1. toad$move()
 3. australia_raster - fixed_cells
 5. raster::setValues(...)
Execution halted
