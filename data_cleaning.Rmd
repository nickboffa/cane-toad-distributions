...
## Convert .txt raster data to .asc

To load the raster .txt files as raster layers easily, we need to convert them to .asc.
```{r}
filenames <- c("raw_data/climate/mean_winter_temp",
              "raw_data/climate/rel_humidity_9am",
              "raw_data/climate/mean_winter_rainfall",
              "raw_data/climate/mean_summer_rainfall")

for (filename in filenames) {
  data <- readLines(paste0(filename, ".txt")) 
  writeLines(data, paste0(filename, ".asc"))
}
```

## Load raster layers and make brick
...
for (raster_layer in rasters) {
  common_extent <- intersect(common_extent, extent(raster_layer))
}
